...
def _gen_rxip_sp(
    ...
    Produces two subckts:
      .subckt rx PAD VDD VSS out   — 2-stage inverter (pre-amp + buffer)
      .subckt rxip PAD_0…N-1 OUT_0…N-1 VDD VSS — N-lane wrapper
    """
    if spec is None:
        spec = DeviceSpec()

    mos_lines = _gen_rx_mos_lines(
        w_preamp_n, w_preamp_p, w_buf_n, w_buf_p,
        nmos_name, pmos_name, l_um, nf, spec,
    )
    # Convert Spectre '//' comments to SPICE '*' comments
    mos_lines = [("* " + l[3:]) if l.startswith("//") else l for l in mos_lines]

    lines = ["* RX pre-amp + buffer — generated by CLIPGen", ""]
    lines.append(".subckt rx PAD VDD VSS out")
    if use_termination:
        lines.append(f"R_bias_hi vterm VDD {r_bias_hi_ohm:.0f}")
        lines.append(f"R_bias_lo vterm VSS {r_bias_lo_ohm:.0f}")
        lines.append(f"RT PAD vterm {r_rx_ohm:.4f}")
    lines.extend(mos_lines)
    lines += [".ends rx", ""]

    in_pins  = [f"PAD_{i}" for i in range(lane_count)]
    out_pins = [f"OUT_{i}" for i in range(lane_count)]
    port_str = " ".join(in_pins) + " " + " ".join(out_pins) + " VDD VSS"
...
def _gen_netlist_charlib(cfg, ch_result, term_result, run_dir: str,
    ...
    # Write model.sp (strip Spectre-only 'simulator lang' directive)
...
cells:
  rxip:
    ...
    functions: {func_str}
"""
    _write(rx_dir, "charlib.yaml", yaml_text)

    # Run CharLib
    print(f"  [RX/charlib] Running CharLib in {rx_dir} ...")
    env = os.environ.copy()
    ...
    return RxNetlistResult(
        ...
        use_termination     = use_term,
        r_rx_ohm            = r_rx,
        c_ac_pF             = getattr(term_result, 'c_ac_pF', 0.0),
        r_bias_hi_ohm       = r_bias_hi,
        r_bias_lo_ohm       = r_bias_lo,
        w_preamp_n_um       = rx_cfg.w_preamp_n_um,
        w_preamp_p_um       = rx_cfg.w_preamp_p_um,
...
def _gen_netlist_liberate(cfg, ch_result, term_result, run_dir: str,
               tx_result=None) -> RxNetlistResult:
    """
